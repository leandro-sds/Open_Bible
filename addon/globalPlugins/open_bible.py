...
def _log_exception(msg: str, exc: Exception | None = None):
	"""Registra mensagens/erros no log do NVDA sem risco de recursão.

	Observação: em muitos pontos do código chamamos esta função dentro de um bloco `except`.
	Nesses casos, passe `exc` (a exceção capturada) para registrar o traceback quando possível.
	"""
	if not log:
		return
	try:
		# Se `exc` foi fornecida, registre com traceback.
		if exc is not None:
			try:
				log.error(msg, exc_info=exc)
			except TypeError:
				# Alguns handlers/loggers podem não aceitar `exc_info` como exceção diretamente.
				log.error(f"{msg}: {exc}")
		else:
			log.error(msg)
	except Exception:
		# Último recurso: não tente registrar novamente para evitar loop/recursão.
		pass

def _ui_message(msg: str):
	"""Envia uma mensagem ao usuário via NVDA, quando disponível."""
	if ui:
		try:
			ui.message(msg)
		except Exception as e:
			_log_exception(f"Exceção ignorada: {e}")
def get_nvda_user_config_dir():
	if nvdaConfig and hasattr(nvdaConfig, "getUserDefaultConfigPath"):
		try:
			...
			_log_exception(f"Falha ao obter pasta de configuração do NVDA: {e}")
	try:
		import globalVars
		if hasattr(globalVars, "appArgs") and hasattr(globalVars.appArgs, "configPath"):
			return globalVars.appArgs.configPath
	except Exception as e:
		_log_exception(f"Falha ao obter configPath do NVDA: {e}")
...
class ConfigManager:
	...
	def _save_direct(self, data):
		try:
			...
			if ui: ui.message("Open Bible: Não foi possível salvar a configuração.")
...
class FavoritesManager:
	...
	def _save(self):
		try:
			...
			if ui: ui.message(f"Erro ao salvar favoritos: {e}")
...
class ReadChaptersManager:
	...
	def _save(self):
		try:
			...
			_log_exception(f"Erro ao salvar histórico de capítulos lidos: {e}")
...
class BibleManager:
	...
	def _detectarVersoes(self):
		...
		try:
			...
			_log_exception(f"Falha ao detectar versões de Bíblia: {e}")
		return versoes
	...
	def adicionar_arquivo_json(self, origem):
		"""Importa um arquivo JSON de Bíblia para a pasta do add-on.

		Valida o conteúdo básico para evitar arquivos inválidos e evita sobrescrever arquivos existentes.
		"""
		if not os.path.isfile(origem) or not origem.lower().endswith(".json"):
			raise ValueError("Arquivo inválido. Selecione um arquivo .json.")

		# Valida o JSON antes de copiar.
		try:
			with open(origem, "r", encoding="utf-8") as f:
				dados = json.load(f)
		except Exception as e:
			raise ValueError(f"Não foi possível ler o JSON: {e}")

		if not isinstance(dados, list) or not dados:
			raise ValueError("O JSON não parece conter uma lista de versículos.")

		# Checagem mínima de estrutura (não altera o padrão de pesquisa; apenas valida importação).
		exemplo = dados[0]
		if not isinstance(exemplo, dict) or not {"livro", "capitulo", "versiculo", "texto"} <= set(exemplo.keys()):
			raise ValueError("Formato inválido. Cada item deve conter: livro, capitulo, versiculo e texto.")

		os.makedirs(self.bibliasDir, exist_ok=True)

		nomeArquivo = os.path.basename(origem)
		base = nomeArquivo[:-5]
		# Garante que não sobrescreva arquivo existente.
		destino = os.path.join(self.bibliasDir, nomeArquivo)
		if os.path.exists(destino):
			# Tenta um nome alternativo (base_2.json, base_3.json, ...)
			i = 2
			while True:
				alt = f"{base}_{i}.json"
				destino = os.path.join(self.bibliasDir, alt)
		...
		nome = base.split("_", 1)[-1] if "_" in base else base
		# Evita colisão de nome de versão na lista.
		if nome in self.versoes:
			i = 2
			nomeBase = nome
			while f"{nomeBase} ({i})" in self.versoes:
				i += 1
			nome = f"{nomeBase} ({i})"
...
class NotesManager:
	...
	def _save(self):
		try:
			...
			if ui: ui.message(f"Erro ao salvar notas da versão {self.versao}: {e}")
...
NOMES_LIVROS = {
	...
	"Jó": "Jó", "Sl": "Salmos", "Pv": "Provérbios", "Ec": "Eclesiastes", "Ct": "Cânticos dos Cânticos",
...
class BibliaFrame(wx.Frame):
	...
	def _sequenciaBoasVindasSeNecessario(self):
		if self._precisaBoasVindas and self.nivel == "livros":
			try:
				...
				_log_exception(f"Falha ao cancelar fala na sequência de boas-vindas: {e}")
		...
		if fd.ShowModal() == wx.ID_OK:
			...
			try:
				with zipfile.ZipFile(path, 'r') as zf:
					...
					zf.extractall(NVDA_CONFIG_BASE)
	...
	def _onPaginaAnteriorClick(self):
		if self.nivel == "busca":
			self.paginaAnterior()
		elif self.nivel == "favoritos":
			self.paginaFavoritosAnterior()
		elif self.nivel == "lidos":
			self.paginaLidosAnterior()

	def _onPaginaProximaClick(self):
		if self.nivel == "busca":
			self.paginaProxima()
		elif self.nivel == "favoritos":
	...
	def _onCloseSaveLastPosition(self, event):
		...
		try:
			...
			if self.livroAtual and self.capituloAtual:
				self.configManager.set_last_position(self.versaoAtual, self.livroAtual, self.capituloAtual)
		except Exception as e:
			_log_exception(f"Exceção ignorada: {e}")
		event.Skip()
	...
	def mostrarLivros(self):
		...
		try:
			...
			_log_exception(f"Exceção ignorada: {e}")
		self.SetTitle(f"Open Bible – {self.versaoAtual}")
		self._atualizarContexto(livro=None, capitulo=None)
		wx.CallAfter(self._ensureListFocus)
		self.leituraIndice = 0
		self._leituraTotalVersos = 0
		
		self._setLeituraText("Open Bible\n\nSelecione um livro.")
	...
	def mostrarCapitulos(self, livro):
		...
		self._updateButtonsForLevel()
		nomeLivro = NOMES_LIVROS.get(livro, livro)
		self.SetTitle(f"Open Bible – {self.versaoAtual} – {nomeLivro}")
	...
	def mostrarVersiculos(self, livro, capitulo):
		...
		nomeLivro = NOMES_LIVROS.get(livro, livro)
		self.SetTitle(f"Open Bible – {self.versaoAtual} – {nomeLivro} {capitulo}")
	...
	def _pararLeituraSeAtiva(self):
		"""Para a leitura contínua com segurança."""
		try:
			if self.leituraTimer.IsRunning():
				self.leituraTimer.Stop()
		except Exception as e:
			_log_exception(f"Falha ao parar o temporizador da leitura contínua: {e}")
		self.leituraAtiva = False
		try:
			if speech:
				speech.cancelSpeech()
		except Exception:
			# Cancelar fala é opcional; não interrompe o fluxo se falhar.
			pass
	...
	def _falarECalcularProximo(self):
		...
		total_versos = self._leituraTotalVersos or len(self._versosLista)
		# Se o conteúdo mudou durante a leitura (troca de capítulo/nível), interrompe com emphasize.
		if self.nivel != "versiculos" or total_versos <= 0:
			self._pararLeituraSeAtiva()
			return
		...
		# Valida se a lista ainda contém os itens esperados.
		try:
			if self.lista.GetCount() < total_versos:
				self._pararLeituraSeAtiva()
				self.anunciar("Leitura interrompida: a lista mudou.")
				return
		except Exception:
			# Se não conseguir consultar a lista, para para evitar loop/travamento.
			self._pararLeituraSeAtiva()
			return

		try:
			self.lista.SetSelection(self.leituraIndice)
			self.lista.EnsureVisible(self.leituraIndice)
			self.lista.SetFocus()
			...
			_log_exception(f"Falha durante a leitura contínua: {e}")
			self._pararLeituraSeAtiva()

	def _onLeituraTick(self, event):
		# Garante que o temporizador não rode após fechamento/destruição.
		try:
	...
	def _setLeituraText(self, texto: str):
		# Atualização leve do campo de leitura para evitar trabalho repetido durante navegação.
		try:
			if getattr(self, "_ultimoTextoLeitura", None) == texto:
				return
			self._ultimoTextoLeitura = texto
			self.txtLeitura.Freeze()
			# ChangeValue evita disparar eventos de texto e costuma ser mais leve quando disponível.
			if hasattr(self.txtLeitura, "ChangeValue"):
				self.txtLeitura.ChangeValue(texto)
			else:
				self.txtLeitura.SetValue(texto)
			self.txtLeitura.ShowPosition(0)
			self.txtLeitura.Thaw()
		except Exception as e:
			_log_exception(f"Exceção ignorada: {e}")
	...
	def _toggleMarkCurrentItem(self):
		if self.nivel == "livros":
			if ui: ui.message("Seleção não disponível no índice de livros.")
			return
		idx = self.lista.GetSelection()
		if idx == wx.NOT_FOUND:
			return
		...
		if self.nivel == "versiculos":
			...
			if self._versosSelecionados:
				...
				if idx == wx.NOT_FOUND or idx >= len(self._versosLista):
					if ui: ui.message("Selecione um versículo para copiar")
					return
			...
			if not linhas:
				if ui: ui.message("Nada para copiar.")
				return
			textoCopiar = "\n".join(linhas)
			...
			if ui: ui.message("Copiar não disponível no índice de livros.")
			return
	...
	def adicionarNota(self):
		if self.nivel != "versiculos":
			if ui: ui.message("Abra um capítulo para adicionar notas")
			return

		dlg = wx.Dialog(self, title=f"Adicionar Nota – {self.versaoAtual}")
		vbox = wx.BoxSizer(wx.VERTICAL)
	...
	def removerNota(self):
		if self.nivel != "versiculos":
			if ui: ui.message("Abra um capítulo para remover notas")
			return
		...
		if notaIdx < 0 or notaIdx >= len(self._notasLista):
			if ui: ui.message("Selecione uma nota para remover")
			return
			...
			try:
				...
				try:
					...
					if not self._notasLista and self._temHeaderNotas:
						...
						self._temHeaderNotas = False
					if ui: ui.message("Nota removida com sucesso")
					self._renderLeituraCapitulo(self.livroAtual, self.capituloAtual,
												[self._formatVersoLine(v, any(s is v for s in self._versosSelecionados)) for v in self._versosLista],
												self._notasLista)
				except Exception:
				...
				if ui: ui.message(f"Erro ao remover nota: {e}")
		dlg.Destroy()


	def buscar(self):
		"""Abre o diálogo de busca e executa a pesquisa sem travar a interface."""
		self._pararLeituraSeAtiva()
		dlg = wx.Dialog(self, title=f"Pesquisar na Bíblia – {self.versaoAtual}")
		vbox = wx.BoxSizer(wx.VERTICAL)
		...
		try:
			...
			displayByKey = {key: NOMES_LIVROS.get(key, key) for key in self.bibleManager.bible_tree.keys()}
			...
			if not termo:
				self.anunciar("Digite um termo para pesquisar.")
				return
			...
			# Mantém o padrão atual da pesquisa (normalização/opções e regex).
			...
			if filtroLivroKey:
				fonte = self.bibleManager.bible_tree.get(filtroLivroKey, {})
				iterator_fonte = (v for caps in fonte.values() for v in caps)
				total = sum(len(caps) for caps in fonte.values())
			else:
				iterator_fonte = (v for lista_versos in self.bibleManager.indexPorLivro.values() for v in lista_versos)
				total = sum(len(lista_versos) for lista_versos in self.bibleManager.indexPorLivro.values())

			if total <= 0:
				self.anunciar("Nenhum texto disponível para pesquisar.")
				return

			progress = wx.ProgressDialog(
				"Pesquisar na Bíblia",
				"Pesquisando... (Esc para cancelar)",
				maximum=total,
				parent=self,
			...
			def finalizar():
				try:
					...
					_log_exception(f"Exceção ignorada: {e}")
				if cancelado:
					self.anunciar("Pesquisa cancelada.")
					return

				self._push_navigation_state()
				self.resultadosBusca = resultados
				self.paginaAtual = 0
				self._primeiraPaginaBuscaCarregada = False
				self.mostrarResultadosBusca()

			def processar_lote():
				nonlocal processados, cancelado

				if cancelado:
					finalizar()
					return

				# Processa em lotes pequenos para manter a interface responsiva.
				lote = 600
				for _ in range(lote):
					try:
						v = next(iterator_fonte)
					except StopIteration:
						finalizar()
						return
					except Exception as e:
						cancelado = True
						_log_exception(f"Erro durante a pesquisa: {e}")
						finalizar()
						return
					...
					try:
						...
						# Não altera a lógica da pesquisa; apenas evita travamentos em dados inesperados.
						_log_exception(f"Erro ao processar versículo durante a pesquisa: {e}")

				try:
					cont, continuar = progress.Update(min(processados, total))
					if not cont:
						cancelado = True
				except Exception:
					# Se o diálogo falhar por algum motivo, segue sem progress.
					pass

				if processados >= total:
					finalizar()
				else:
					wx.CallLater(1, processar_lote)

			wx.CallAfter(processar_lote)
			...
			try:
				...
				_log_exception(f"Exceção ignorada: {e}")
			_log_exception(f"Falha ao abrir/executar a pesquisa: {e}")
			self.anunciar("Não foi possível realizar a pesquisa nesta versão.")
	...
	def mostrarResultadosBusca(self):
		...
		# Na primeira página (carregamento inicial), ao inserir itens na lista com o foco nela,
		# o NVDA pode anunciar resultados e interromper a mensagem do total encontrado.
		# Para manter o comportamento de pesquisa e apenas melhorar a fala, desviamos o foco,
		# desabilitamos temporariamente a lista durante o preenchimento e só selecionamos/focalizamos depois do anúncio.
		primeiraPagina = (self.paginaAtual == 0 and not getattr(self, "_primeiraPaginaBuscaCarregada", False))
		if primeiraPagina:
			self._primeiraPaginaBuscaCarregada = True
			try:
				# Evita anúncios do controle de resultados enquanto popula a lista.
				self.lista.Disable()
				if getattr(self, "txtLeitura", None):
					self.txtLeitura.SetFocus()
				else:
					self.SetFocus()
			except Exception as e:
		...
		# Na primeira página, não selecione imediatamente (seleção dispara anúncio e pode cortar a frase do total).
		if primeiraPagina:
			try:
				self.lista.SetSelection(wx.NOT_FOUND)
			except Exception as e:
		...
		self.SetTitle(
			f"Open Bible – {self.versaoAtual} – Resultados da busca (página {self.paginaAtual+1} de {totalPaginas+1})"
		)

		msg = f"Foram encontrados {total} versículos. Página {self.paginaAtual+1} de {totalPaginas+1}."
		if ui:
			if primeiraPagina:
				# Se alguma fala do controle entrou na fila, limpe antes de anunciar o total.
				try:
					if speech:
						speech.cancelSpeech()
				except Exception as e:
					_log_exception(f"Falha ao cancelar fala antes do anúncio do total: {e}")
		...
		if primeiraPagina:
			# Reabilita e foca a lista somente após o anúncio do total para não interromper a fala.
			def _habilitar_e_selecionar_primeiro():
				try:
					# Se o usuário saiu do modo "busca" antes do atraso, não faça nada.
					if getattr(self, "nivel", None) != "busca":
						return
					try:
						self.lista.Enable()
					except Exception:
						pass
					if self.lista and self.lista.GetCount() > 0:
						self.lista.SetSelection(0)
						self.lista.EnsureVisible(0)
						self.lista.SetFocus()
				except Exception as e:
					_log_exception(f"Exceção ignorada: {e}")
			# Tempo suficiente para a frase curta ser falada, sem atrasar demais a navegação.
			wx.CallLater(1200, _habilitar_e_selecionar_primeiro)
		else:
			wx.CallAfter(self._ensureListFocus)
		...
		if self.nivel == "versiculos":
			if self._versosSelecionados:
				...
				if idx == wx.NOT_FOUND or idx >= numVersos:
					if ui: ui.message("Selecione um versículo para favoritar")
					return
				v = self._versosLista[idx]
				items.append({
					"livro": self.livroAtual,
			...
			idx = self.lista.GetSelection()
			if idx == wx.NOT_FOUND:
				if ui: ui.message("Selecione um resultado para favoritar")
				return
			inicio = self.paginaAtual * self.itensPorPagina
			realIdx = inicio + idx
			if 0 <= realIdx < len(self.resultadosBusca):
				...
				if ui: ui.message("Seleção inválida para favoritar")
				return
		else:
			if ui: ui.message("Ctrl+F funciona em capítulos (versículos) e nos resultados da busca")
			return
	...
	def removerFavoritoAtual(self):
		...
		if 0 <= realIdx < len(self.favoritos):
			...
			if wx.MessageBox(f"Remover favorito?\n\n{self._formatFavoritoLinha(fav)}", "Confirmar remoção", wx.YES_NO | wx.ICON_QUESTION) == wx.YES:
				...
				self.mostrarFavoritos()
				if ui: ui.message("Favorito removido")
	...
	def mostrarFavoritos(self):
		...
		self.SetTitle(f"Open Bible – {self.versaoAtual} – Favoritos (página {self.favPaginaAtual+1} de {totalPaginas+1})")
		if ui:
	...
	def mostrarCapitulosLidos(self):
		...
		self.SetTitle(f"Open Bible – {self.versaoAtual} – Capítulos Lidos (página {self.lidosPaginaAtual+1} de {totalPaginas+1})")
		if ui:
	...
	def onKeyDown(self, event):
		...
		if self.nivel == "busca":
			if keyCode == wx.WXK_PAGEUP and no_modifiers:
				self.paginaAnterior(); return
			if keyCode == wx.WXK_PAGEDOWN and no_modifiers:
				self.paginaProxima(); return
		if self.nivel == "favoritos":
			if keyCode == wx.WXK_PAGEUP and no_modifiers:
				self.paginaFavoritosAnterior(); return
			if keyCode == wx.WXK_PAGEDOWN and no_modifiers:
				self.paginaFavoritosProxima(); return
			if keyCode == wx.WXK_DELETE and no_modifiers:
				self.removerFavoritoAtual(); return
		if self.nivel == "lidos":
			if keyCode == wx.WXK_PAGEUP and no_modifiers:
	...
	def onChar(self, event):
		...
		no_modifiers = not ctrl and not shift and not alt
		# Atalho rápido: não processar teclas de navegação que não são tratadas aqui.
		if no_modifiers and keyCode in (wx.WXK_UP, wx.WXK_DOWN, wx.WXK_PAGEUP, wx.WXK_PAGEDOWN, wx.WXK_HOME, wx.WXK_END):
			event.Skip(); return
	...
	def _onListSelectionChanged(self, event):
		if self.nivel == "versiculos":
			...
			if idx != wx.NOT_FOUND and idx < numVersos:
				v = self._versosLista[idx]
				try:
					self._setLeituraText(f"{v['versiculo']}: {v['texto']}")
					if not self.leituraAtiva:
			...
			if idx != wx.NOT_FOUND:
				try:
					self._setLeituraText(self._strip_prefix(self.lista.GetString(idx)))
				except Exception as e:
					_log_exception(f"Exceção ignorada: {e}")
		elif self.nivel == "capitulos":
			idx = self.lista.GetSelection()
			if idx != wx.NOT_FOUND:
				try:
					capStr = self._strip_prefix(self.lista.GetString(idx)).strip()
					cap = int(capStr)
	...
	def _compararVersiculoSelecionado(self):
		self._pararLeituraSeAtiva()
		if self.nivel != "versiculos":
			if ui: ui.message("Abra um capítulo e selecione um versículo para comparar.")
			return
		idx = self.lista.GetSelection()
		numVersos = len(self._versosLista)
		if idx == wx.NOT_FOUND or idx >= numVersos:
			if ui: ui.message("Selecione um versículo para comparar.")
			return
		...
		def onCheck(evt):
			...
			if len(marcados) > 10:
				...
				if ui: ui.message("Limite de seleção atingido.")
				evt.Skip()
				return
			
			try:
				estado = "Marcada" if clb.IsChecked(idxEvt) else "Desmarcada"
				if ui: ui.message(estado)
			except Exception as e:
				_log_exception(f"Exceção ignorada: {e}")
			evt.Skip()
		...
		if dlg.ShowModal() == wx.ID_OK:
				...
				try:
					...
					for v in bib:
						if (safe_compare(v.get("livro"), livro) or 
							(v.get("livro", "").strip().lower() == NOMES_LIVROS.get(livro, "").lower())) and \
						   safe_compare(v.get("capitulo"), cap) and \
						   safe_compare(v.get("versiculo"), vers):
							texto = v.get("texto")
							break
			...
			def _abrirVersaoEscolhida():
				...
				try:
					...
					try:
						pos = [v["versiculo"] for v in self.indexPorLivro[livro] if v["capitulo"] == cap].index(vers)
						self.lista.SetSelection(pos)
						self.lista.SetFocus()
						nomeLivro = NOMES_LIVROS.get(livro, livro)
			...
			def _resCharHook(e):
				...
				if code == wx.WXK_SPACE:
					...
					if sel != wx.NOT_FOUND:
						if sel in selecionados_indices:
							...
							if ui: ui.message("Item desmarcado")
						else:
							selecionados_indices.add(sel)
							if ui: ui.message("Item marcado")
						refresh_lines()
						try:
							lb.SetSelection(sel)
						except Exception as e:
				...
				if ctrl and not alt and not shift and code == ord('A'):
					...
					refresh_lines()
					if ui: ui.message("Todos os itens foram marcados")
					return

				if ctrl and not alt and not shift and code == ord('Z'):
					selecionados_indices = set()
					refresh_lines()
					if ui: ui.message("Seleção limpa")
					return
					...
					for i in indices_para_copiar:
						if 0 <= i < len(compEntries):
							...
							if ent["texto"] is not None:
								entradas.append(ent)
					if not entradas:
						if ui: ui.message("Nenhum versículo válido para copiar.")
						return
	...
	def _updateButtonsForSearch(self):
		...
		totalPaginas = (len(self.resultadosBusca) - 1) // self.itensPorPagina if self.resultadosBusca else 0
		if self.paginaAtual > 0: self.btnPagAnterior.Enable()
		else: self.btnPagAnterior.Disable()
		if self.paginaAtual < totalPaginas: self.btnPagProxima.Enable()
		else: self.btnPagProxima.Disable()
		self.btnBuscar.Enable()
		self.btnFavoritos.Enable()
	...
	def _updateButtonsForFavorites(self):
		...
		self.btnRemoverNota.Disable()
		totalPaginas = (len(self.favoritos) - 1) // self.favItensPorPagina if self.favoritos else 0
		if self.favPaginaAtual > 0: self.btnPagAnterior.Enable()
		else: self.btnPagAnterior.Disable()
		if self.favPaginaAtual < totalPaginas: self.btnPagProxima.Enable()
		else: self.btnPagProxima.Disable()
		self.btnLimparBusca.Disable()
		self.btnBuscar.Enable()
		self.btnFavoritos.Enable()
	...
	def _updateButtonsForLidos(self):
		...
		self.btnRemoverNota.Disable()
		totalPaginas = (len(self.lidosLista) - 1) // self.lidosItensPorPagina if self.lidosLista else 0
		if self.lidosPaginaAtual > 0: self.btnPagAnterior.Enable()
		else: self.btnPagAnterior.Disable()
		if self.lidosPaginaAtual < totalPaginas: self.btnPagProxima.Enable()
		else: self.btnPagProxima.Disable()
		self.btnLimparBusca.Disable()
		self.btnBuscar.Enable()
		self.btnFavoritos.Enable()
	...
	def abrirGerenciadorBiblias(self):
		self._pararLeituraSeAtiva()
		dlg = wx.Dialog(self, title=f"Gerenciar Bíblias – {self.versaoAtual}", style=wx.DEFAULT_DIALOG_STYLE | wx.RESIZE_BORDER)
		vbox = wx.BoxSizer(wx.VERTICAL)
		...
		def onImportar(e):
			...
			if fd.ShowModal() == wx.ID_OK:
				...
				try:
					...
					refreshList()
					if ui: ui.message("Bíblia importada com sucesso.")
		...
		def onRemover(e):
			...
			if wx.MessageBox(f"Remover a versão '{versao}'?", "Confirmar", wx.YES_NO | wx.ICON_QUESTION) == wx.YES:
				try:
					...
					refreshList()
					if ui: ui.message("Versão removida.")
				except Exception as err:
		...
		def onTornarPadrao(e):
			...
			try:
				...
				self._ultimoCapituloSelecionado = None
				self.mostrarLivros()
				if ui: ui.message(f"Versão atual: {versao}")
				dlg.Close()
			except Exception as err:
	...
	def _iniciar_interface(self):
		try:
			...
			if not bm.has_versions():
				msg = "Nenhuma versão encontrada. Coloque arquivos JSON em 'biblias/' (ex.: 01_ACF.json, 02_NVI.json)."
				_ui_message(msg)
				# Evite diálogos modais diretamente no fluxo do script (pode aparentar travamento).
				try:
					if gui and hasattr(gui, "messageBox"):
						wx.CallAfter(gui.messageBox, msg, "Open Bible", wx.OK | wx.ICON_WARNING)
				except Exception as e:
					_log_exception("Falha ao exibir aviso de nenhuma versão encontrada", e)
				return
			...
			try:
				...
				biblia = []
				if ui: ui.message(f"Erro ao carregar versão '{versao}': {e}")
			...
			_log_exception("Erro crítico ao iniciar o Open Bible", e)
			_ui_message(f"Erro ao iniciar o Open Bible: {e}")
