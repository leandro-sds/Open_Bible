...
class BibliaFrame (wx .Frame ):
			...
			if res ==wx .ID_OK :
				...
				if termo :
					...
					try :
						...
						if ui :ui .message ("Erro durante a pesquisa.")
						return 
	...
	def mostrarResultadosBusca (self ):
		...
		self .SetTitle (f"Open Bible – {self.versaoAtual} – Resultados da busca (página {self.paginaAtual+1} de {totalPaginas+1})")
	...
	def _iniciar_interface (self ):
		try :
			...
			if not bm .has_versions ():
				if ui :ui .message ("Nenhuma versão encontrada. Coloque arquivos JSON em 'biblias/' (ex.: 01_ACF.json, 02_NVI.json).")
				dlg =wx .MessageDialog (None ,"Nenhuma versão encontrada na pasta 'biblias/'.","Open Bible",wx .OK |wx .ICON_WARNING )
				dlg .ShowModal ()
				dlg .Destroy ()
				return 
